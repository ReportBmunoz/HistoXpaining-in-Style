...
def get_prob_diff(generator, classifier, eigvec, filterd_idx, latents, prob, args):
    ...
    for eigenvec_idx in range(args.nof_eigenv):
        ...
        for idx in tqdm(filterd_idx):
            ...
            # Debug print
            print(f"\nOriginal logit: {logits[idx]}")
            print(f"Changed logit: {changed_logit}")

            changed_prob = [torch.sigmoid(l) for l in changed_logit]

            # Debug print
            print(f"Original prob: {prob[idx]}")
            print(f"Changed prob: {changed_prob}")
    ...
    for (eigen_idx, eigen_direct), diff in most_significant_eigendirect.items():
        ...
        if args.prob_thre:
            for image_idx in filterd_image_idx:
                ...
                print(f"Direction: {eigen_direct}, Sign idx: {sign_idx}")
                print(f"Before change (class {args.class_label}): {float(prob_before_change):.6f}")
                print(f"After change (class {args.class_label}): {float(prob_after_change):.6f}")
                print(f"Change amount: {float(prob_change):.6f}")
                ...
                if ((float(prob_before_change) < 0.5) and (float(prob_after_change) > 0.5)) or \
                    ...
                    print("✓ Image EXPLAINED - crossed 0.5 threshold")
                else:
                    print("✗ Image NOT explained - did not cross threshold")
...
def get_probablities(set_logits, set_logits_diffs, class_label):
    ...
    # print("Logits diffs:", set_logits_diffs) 
    # print("Logits after:", logits_sum)
    # print("Probs before:", prob_before)
    # print("Probs after:", prob_after)

    # Modificar para usar las probabilidades calculadas directamente
    prob_before = []
    prob_after = []
    
    for logit, diff in zip(set_logits, set_logits_diffs):
        prob_before.append(torch.sigmoid(logit[class_label]))
        prob_after.append(torch.sigmoid(logit[class_label] + diff))
        
        print(f"\nLogit: {logit[class_label]}")
        print(f"Diff: {diff}")
        print(f"Prob before: {prob_before[-1]}")
        print(f"Prob after: {prob_after[-1]}")
...
def save_changed_images(result_explained_images, generator, discriminator, eigvec, logits, latents,
    ...
    for (eigen_idx, eigen_direct), exp_image_idx in result_explained_images.items():
        ...
        prob_changes = [exp_image_idx.get(key) for key in image_idx_set]
        prob_before, prob_after = get_probablities(set_logits, prob_changes, args.class_label)
