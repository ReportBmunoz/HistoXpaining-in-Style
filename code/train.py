...
# def mixing_noise(batch, latent_dim, prob, device):
#     """Generate mixing noise for StyleGAN2"""
#     if prob > 0 and random.random() < prob:
#         noise1 = torch.randn(batch, latent_dim, device=device)
#         noise2 = torch.randn(batch, latent_dim, device=device)
...
def train_encoder(args, loader, generator, g_optim, g_ema, device, classifier, encoder, e_optim, ckpt):
    ...
    accum = 0.7 ** (32 / (10 * 1000))

    sample_z = torch.randn(args.n_sample, args.latent, device=device)

    # Ajustar learning rate del generador (problema de ruido encoder)
    ...
    for idx in pbar:
        ...
        # Añadir regularización de suavidad
        dx = fake_img[:,:,:,1:] - fake_img[:,:,:,:-1]
        dy = fake_img[:,:,1:,:] - fake_img[:,:,:-1,:]
        ...
        # reconstruct_loss = reconstruct_loss_x*0.1 + reconstruct_loss_w*0.1 + reconstruct_loss_lpips*0.8

        # Nuevos pesos para las pérdidas
        reconstruct_loss = (
            reconstruct_loss_x * 0.4 +      # Aumentar peso de L1 pixel-wise
            reconstruct_loss_w * 0.1 +      # Mantener peso de latent
            reconstruct_loss_lpips * 0.4 +  # Reducir peso de LPIPS
            reg_loss * 0.1                  # Añadir regularización de suavidad
        )
        ...
        identity_loss = F.mse_loss(encoder(fake_img), real_encoded.detach())
        # total_loss = 0.15 * class_loss + 0.6* reconstruct_loss + 0.25*identity_loss
        total_loss = 0.1 * class_loss + 0.8 * reconstruct_loss + 0.1 * identity_loss

        
        # total_loss = class_weight * class_loss + reconstruct_weight * reconstruct_loss
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, ckpt):
    ...
    for idx in pbar:
        ...
        if cgan:
            ...
            real_label = real_label.to(torch.long)
...
# def ssim_loss(fake_img, real_img):
#     # Compute SSIM
#     ssim_value = ssim_metric(fake_img, real_img)
#     # SSIM is a similarity measure, so we want to minimize the loss
#     return 1 - ssim_value  # Invert SSIM to create a loss
...
def train_stylechexplain(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, classifier, encoder, e_optim, ckpt):
    ...
    # ssim_metric = StructuralSimilarityIndexMeasure(data_range=1.0).to(device)  # Adjust data_range based on your image normalization

    # Add warmup parameters
    # warmup_iters = 10000  # First N iterations for reconstruction only
    ...
    #     if i < warmup_iters:
    #         # During warmup: only train encoder and generator for reconstruction
    #         requires_grad(generator, True)
    ...
    #         # Compute reconstruction losses with higher weights
    #         fake_encoded = encoder(fake_img)
    #         reconstruct_loss_x = F.l1_loss(fake_img, real_img)
    #         reconstruct_loss_w = F.l1_loss(fake_encoded, real_encoded)
    #         reconstruct_loss_lpips = lpips(fake_img/fake_img.max(), real_img/real_img.max(), net_type='alex', version='0.1').flatten()

    #         # Stronger reconstruction weights during warmup
    #         reconstruct_loss = reconstruct_loss_x * 0.4 + reconstruct_loss_w * 0.3 + reconstruct_loss_lpips * 0.3
    ...
    for idx in pbar:
        ...
        reconstruct_loss_w = F.l1_loss(fake_encoded, real_encoded)
        reconstruct_loss_lpips = lpips(fake_img/fake_img.max(), real_img/real_img.max(), net_type='alex', version='0.1').flatten()
        # reconstruct_loss_ssim = 1 - ssim_value  # Invert SSIM to create a loss

        # reconstruct_loss_ssim = ssim_loss(fake_img, real_img)

        # reconstruct_loss = reconstruct_loss_x + reconstruct_loss_w + reconstruct_loss_lpips + reconstruct_loss_ssim
        # reconstruct_loss = reconstruct_loss_x*0.2 + reconstruct_loss_w*0.2 + reconstruct_loss_lpips*0.6
        reconstruct_loss = reconstruct_loss_w*0.3 + reconstruct_loss_lpips*0.7
        loss_dict["reconstruct_loss"] = reconstruct_loss
        loss_dict["reconstruct_loss_x"] = reconstruct_loss_x
        loss_dict["reconstruct_loss_w"] = reconstruct_loss_w
        loss_dict["reconstruct_loss_lpips"] = reconstruct_loss_lpips
        # loss_dict["reconstruct_loss_ssim"] = reconstruct_loss_ssim

        encoder.zero_grad()
        generator.zero_grad()

        identity_loss = F.mse_loss(encoder(fake_img), real_encoded.detach())
        # total_loss = 0.15 * class_loss + 0.85* reconstruct_loss
        total_loss = 0.15 * class_loss + 0.55* reconstruct_loss + 0.3*identity_loss
        total_loss.backward()
        ...
        # reconstruct_loss_ssim_val = loss_dict["reconstruct_loss_ssim"].mean().item()

        if get_rank() == 0:
            pbar.set_description(
                (
                    f"reconstruct_x: {reconstruct_loss_x_val:.3f} "
                    f"reconstruct_w: {reconstruct_loss_w_val:.3f} "
                    f"reconstruct_lpips: {reconstruct_loss_lpips_val:.3f} "
                    f"d: {d_loss_val:.2f}; g: {g_loss_val:.2f}; r1: {r1_val:.2f}; "
                    f"path: {path_loss_val:.2f}; mean path: {mean_path_length_avg:.2f}; "
                    f"augment: {ada_aug_p:.4f}; class: {class_loss_val:.2f}; "
                    f"reconstruct: {reconstruct_loss_val:.3f} "
                )
            )
            '''
            print(f"d: {d_loss_val:.2f}; g: {g_loss_val:.2f}; r1: {r1_val:.2f}; "
                    ...
                    f"reconstruct_ssim: {reconstruct_loss_ssim_val:.1f} "
                    )
            '''
                ...
                wandb.log(
                    {
                        ...
                        "Reconstruct w": reconstruct_loss_w_val,
                        "Reconstruct LPIPS": reconstruct_loss_lpips_val,
                        # "Reconstruct SSIM": reconstruct_loss_ssim_val,
                    }
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=14,
        help="number of latent codes in W+ space"
    )
    ...
    # args.embedding_size = 32  # embedding for labels in Generator and Discriminator

    # control how often samples and checkpoints are saved
    args.save_samples_every = 500
    args.save_checkpoint_every = 1500
    ...
    if build["E"]:
        encoder = Encoder(
            args.size, channel_multiplier=args.channel_multiplier, output_channels=args.latent, n_latent=args.n_latent
        ).to(device)
        e_optim = optim.Adam(
            encoder.parameters(),
            lr=args.lr
