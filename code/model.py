...
class Generator(nn.Module):
    ...
    def forward(
        ...
        # Handle W+ space input
        if input_is_latent and len(styles[0].shape) == 3:
            latent = styles[0]  # Already in shape [batch_size, n_latent, latent_dim]
        else:
...
class Discriminator(nn.Module):
    ...
    # def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], nof_classes=9, conditional_gan=False):
        super().__init__()
...
#         self.final_linear = nn.Sequential(
#             nn.BatchNorm1d(channels[4] * 4 * 4),
#             EqualLinear(channels[4] * 4 * 4, channels[4]),
#             nn.LeakyReLU(0.2),
#             EqualLinear(channels[4], output_channels)
#         )
...
class Encoder(nn.Module): ##Encoder 18x512
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], output_channels=None, n_latent=14):
        super().__init__()
        ...
        # Modified final layers to output W+ space latents
        self.final_linear = nn.Sequential(
            nn.BatchNorm1d(channels[4] * 4 * 4),
            EqualLinear(channels[4] * 4 * 4, output_channels * n_latent)
        )
    ...
    def forward(self, input):
        ...
        out = out.view(batch, -1)
        out = self.final_linear(out)
        # Reshape to [batch_size, n_latent, latent_dim]
        return out.view(batch, self.n_latent, -1)

# class Encoder(nn.Module):
#     def __init__(self, size, channel_multiplier=2, output_channels=None):
#         super().__init__()
        
#         # Load pretrained ResNet-34 as a backbone
#         resnet = models.resnet34(pretrained=True)
#         self.resnet_features = nn.Sequential(*list(resnet.children())[:-2])  # Remove FC layers
        
#         # Custom final layers to map to desired latent space
#         self.final_conv = nn.Conv2d(512, 512, kernel_size=3, stride=2, padding=1)
#         self.flatten = nn.Flatten()
#         self.final_linear = nn.Sequential(
#             nn.Linear(512 * 4 * 4, 512),
#             nn.LeakyReLU(0.2),
#             nn.Linear(512, output_channels if output_channels else 512)
#         )
